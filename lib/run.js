...
const { getAncestors, savePending, collectPending, cleanupConsumed, cleanupStale } = require('./session')
...
function resolveCoauthor (config, transcriptPath) {
  ...
  // Auto-detect: extract model from transcript (present = Claude authored it)
...
function run (input) {
  ...
  if (sessionId && !hasTrackedModifications(root, sessionId)) {
    savePending(root, sessionId, formattedTranscript)
  ...
  try {
    ...
    // Collect pending transcripts from ancestor sessions
    let combinedBody = body
    if (sessionId) {
      const ancestors = getAncestors(root, sessionId)
      // Include current session's own pending (from earlier skipped turns)
      const allIds = [...ancestors.reverse(), sessionId]
      const pending = collectPending(root, allIds)
      if (pending.length > 0) {
